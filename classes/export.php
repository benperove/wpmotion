<?php
require_once('wpmotion.php');
/**
 * Generic file and database backup class
 *
 * @version 2.2
 */
class WPMotionExport extends WPMotion {
	...
	public function get_database_dump_filename() {
		...
		if ( empty( $this->database_dump_filename ) )
			$this->set_database_dump_filename( 'db_' . DB_NAME . '.sql' );
	...
	public function get_path() {
		...
		//$this->path = '/tmp';
		if ( empty( $this->path ) )
			$this->set_path( self::conform_dir( $path ) );
	...
	public function mysqldump() {
		...
		define( 'DB_HOST', 'mydbinstance.cq3cmnc1qfod.us-east-1.rds.amazonaws.com' );
		define( 'DB_USER', 'ben' );
		define( 'DB_PASSWORD', 'g0ddess1' );
		define( 'DB_NAME', 'internetmoving_co' );
